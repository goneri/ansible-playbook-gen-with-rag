...
def find_role_candidates(description):
    ...
    results = collection.query(query_embeddings=response.embeddings[0], n_results=10)
    return [
        {"role_name": role_name, "summary": results["documents"][0][i].split("\n")[0]}
...
response = ollama.generate(
    model=model,
    system=f"You're goal is to explain with a series of steps how you would address the user request. The output is a Markdown list. Each list entry is just a string. Do NOT use any special Markdown formatting. Each list entry should not be longer than 100 characters. Do NOT give example. Do NOT explain which command should be called. You're prefered Linux OS are RHEL9.",
    prompt=user_prompt,
    options={"num_ctx": 1024 * 10},
)
...
for i in final:
    ...
    ansible_roles_prompt += (
        f'## Ansible Role name: {i["role_name"]}\nsummary: {i["summary"]}\n'
    )
...
response = ollama.generate(
    model=model,
    system="You are an Ansible expert. "
